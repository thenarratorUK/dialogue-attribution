...
def is_single_titlecase_speaker_label(label_text, next_char=""):
    """True for a single-word Titlecase label immediately followed by ':' and then whitespace.

    Matches:
      'Pixel:' + next_char=' '
      'Pixel:' + next_char='\t'
      'Pixel: ' (space included in label_text)
      'Friedrich:' + next_char=' '
    ...
    # Normalise common space-like characters inside the label text.
    s = s.replace("\u00A0", " ").replace("\u202F", " ").replace("\u2009", " ").replace("\u200A", " ").replace("\u200B", "")

    next_c = "" if next_char is None else str(next_char)
    # Treat NBSP as whitespace for next_char too
    if next_c == "\u00A0":
        next_c = " "

    # Whitespace after colon may be inside the same italic run or in the next run (next_char).
    has_ws_after_colon = s.endswith(": ") or s.endswith(":\t") or (s.endswith(":") and next_c[:1].isspace())
    if not has_ws_after_colon:
        return False

    core = s.rstrip()  # remove trailing whitespace
    if not core.endswith(":"):
        return False

    name = core[:-1]  # remove colon
    # Single word: First letter uppercase, rest lowercase, letters only.
...
def extract_italicized_text(paragraph):
    ...
    for run in paragraph.runs:
        if effective_run_italic(run, paragraph):
            ...
            if len(joined.split()) >= 2 or is_single_titlecase_speaker_label(raw_block, next_char=""):
