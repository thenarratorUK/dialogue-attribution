...
def clean_screenplay_speaker(raw: str) -> str:
    """\
    Normalise screenplay speaker names by:
      - stripping any parenthetical segments (e.g. '(V.O.)', '(cont\u2019d)'),
      - collapsing extra whitespace,
      - applying smart_title() for consistent capitalisation.

    Examples:
      'YOUNG NEIL (V.O.)'   -> 'Young Neil'
      'YOUNG NEIL'          -> 'Young Neil'
      'YOUNG NEIL (cont\u2019d)' -> 'Young Neil'
    """
    s = (raw or "").strip()
    if not s:
        return s
    ...
    if not s:
        # Fallback: if we stripped everything, use the original
        s = (raw or "").strip()

    return smart_title(s)


def parse_docx_screenplay(docx_path: str):
    """\
    Parse a screenplay-style DOCX, where character names and dialogue
    live in the same paragraph but on different logical lines, e.g.:
    ...
    Strategy:
      - Split each paragraph at '\\n' into logical lines.
      - The first logical line must 'look like' a character name.
      - The second logical line must contain lowercase letters
        (to avoid treating headings as speakers).
      - All subsequent logical lines with lowercase letters in the
        same paragraph are concatenated into the dialogue text.

    This is designed for screenplay-style DOCX (e.g. Scott Pilgrim)
    and does not replace the radio-play parser.
    """
    import docx
    ...
    def has_lowercase(s: str) -> bool:
        return any(c.islower() for c in s if c.isalpha())

    def looks_like_speaker_line(raw: str) -> bool:
        # Strip leading tabs/spaces
        s = raw.strip().lstrip("\t")
        if not s:
            return False

        # Allow trailing colon but ignore it for the test
        if s.endswith(":"):
            s_core = s[:-1].rstrip()
        else:
            s_core = s

        # Filter out obvious scene headings like "INT. ..." or "EXT. ..."
        if s_core.startswith(("INT.", "EXT.", "INT/EXT.", "EXT/INT.")):
            return False

        tokens = s_core.split()
        letter_tokens = [t for t in tokens if any(c.isalpha() for c in t)]
        if len(letter_tokens) == 0 or len(letter_tokens) > 4:
            return False

        # All alphabetic characters must be uppercase
        return is_all_caps_name(s_core)

    for p in doc.paragraphs:
        text_p = p.text.replace("\r", "")
        parts = text_p.split("\n")
        if len(parts) < 2:
            continue  # must have at least NAME + one more line

        first_raw = parts[0]
        if not looks_like_speaker_line(first_raw):
            continue

        second = parts[1]
        if not has_lowercase(second):
            # Second line must look like dialogue
            continue

        # At this point, treat this paragraph as a dialogue block
        speaker_raw = first_raw.strip()
        dialogue_lines = []
        ...
        for seg in parts[1:]:
            ...
            if not sseg:
                continue
            if has_lowercase(sseg):
                dialogue_lines.append(sseg)

        if dialogue_lines:
            results.append({"speaker": speaker_raw, "text": " ".join(dialogue_lines)})

    return results


def extract_dialogue_from_docx_screenplay(docx_path: str):
    """\
    Build quotes.txt-style lines from a screenplay DOCX.
    ...
    Speaker names are cleaned to remove parentheticals and
    normalised with smart_title().
    """
    pairs = parse_docx_screenplay(docx_path)
    lines = []
    ...
    for idx, pair in enumerate(pairs, start=1):
        ...
        speaker = clean_screenplay_speaker(raw_speaker)
        lines.append(f"{idx}. {speaker}: {text}")

    return lines


def extract_dialogue_from_docx_script_auto(docx_path: str):
    """\
    Decide automatically whether this DOCX looks like:
      - radio-play style (NAME: Dialogue / NAME Dialogue), or
      - screenplay style (NAME on one logical line, dialogue on next).

    Returns:
      (quotes_lines, mode)

    where mode is "radio" or "screenplay".
    """
    ...
    if "docx_bytes" in st.session_state:
        ...
        if st.session_state.get("docx_only", False):
            if st.session_state.get("quotes_lines") is not None:
                ...
                if st.session_state.get("content_type", "Book") == "Script":
                    dialogue_list, script_mode = extract_dialogue_from_docx_script_auto(st.session_state.docx_path)
